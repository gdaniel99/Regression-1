...
```{r}
#To get regression
whwi.mod1 <- lm(quality ~ fixed.acidity + volatile.acidity + citric.acid, data=White_wines)
#To see regression statistics
summary(whwi.mod1)

#Citric acid is removed because p (=0.73) is greater than the significance level of alpha=0.05.
#To get regression
whwi.mod1 <- lm(quality ~ fixed.acidity + volatile.acidity, data=White_wines)
#To see regression statistics
summary(whwi.mod1)

#To get regression
whwi.mod2 <- lm(quality ~ residual.sugar + chlorides + free.sulfur.dioxide + total.sulfur.dioxide, data=White_wines)
#To see regression statistics
summary(whwi.mod2)

#To get regression
whwi.mod3 <- lm(quality ~ density + pH + sulphates + alcohol, data=White_wines)
#To see regression statistics
summary(whwi.mod3)
```
...
If education increases by 1 year, then average prestige rating will increase by 3.73 units, with income and women held constant.